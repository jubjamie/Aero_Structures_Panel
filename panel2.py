...
# Plot graphs
plt.figure(1)
plt.plot(bstRecordAdjust, 'b-', label='bst cm - Solution: ' + str(format(bstRecordAdjust[-1], '.2f')) + 'cm')
plt.plot(tstRecord, 'g--', label='tst mm - Solution: ' + str(format(tstRecord[-1], '.2f')) + 'mm')
plt.plot(tskRecord, 'c-.', label='tsk mm - Solution: ' + str(format(tskRecord[-1], '.2f')) + 'mm')
plt.xlabel('Iteration')
plt.ylabel('Value Attempt', color='k')
plt.yticks(major_ticks1)
plt.tick_params('y', colors='k')
plt.xlim([0, len(bstRecordAdjust)])
plt.legend(loc='upper right', shadow=True)
plt.grid(True)
plt.title("Change of dimensions Over Iterations \nBest Mass per Unit Wing Surface Area: " + str(format(mass(res['x']), '.3f')) + " kg")
plt.savefig('dimensions.png')


plt.figure(2)
plt.plot(massRecord, 'r-', label='Optimisation Mass per\nUnit Wing Surface Area \nSolution Minimum: ' + str(format(mass(res['x']), '.3f')) + 'kg')
plt.xlabel('Iteration')
plt.ylabel('Mass (kg)', color='k')
# plt.yticks(major_ticks1)
plt.tick_params('y', colors='k')
plt.xlim([0, len(massRecord)])
plt.legend(loc='upper right', shadow=True)
plt.grid(True)
plt.title("Change of Unit Mass Over Iterations \nBest Mass per Unit Wing Surface Area: " + str(format(mass(res['x']), '.3f')) + " kg")
plt.savefig('mass.png')

plt.figure(3)
plt.plot(rsfSkinRecord, 'b-', label='Skin Buckling - Solution Value: ' + str(format(rsfSkinRecord[-1], '.2f')))
plt.plot(rsfStiffRecord, 'g--', label='Stiffener Buckling - Solution Value: ' + str(format(rsfStiffRecord[-1], '.2f')))
plt.plot(rsfMatRecord, 'c-.', label='Material Failure - Solution Value: ' + str(format(rsfMatRecord[-1], '.2f')))
plt.plot(rsfEulerRecord, 'k:', label='Euler Buckling - Solution Value: ' + str(format(rsfEulerRecord[-1], '.2f')))
plt.xlabel('Iteration')
plt.xlim([0, len(rsfStiffRecord)])
plt.ylabel('Reserve Factor', color='k')
plt.tick_params('y', colors='k')
plt.legend(loc='upper right', shadow=True)
plt.grid(True)
plt.title("Reserve Factors Over Iterations \nBest Mass per Unit Wing Surface Area: " + str(format(mass(res['x']), '.3f')) + " kg")
plt.savefig('rsf.png')
